...
def poll_for_all_events(interval_minutes=2):
    logger.info("Starting poll_for_all_events")
    while True:  # Infinite loop to keep the polling process running
        logger.info("Polling for status changes")
        poll_for_status_changes(interval_minutes)
        logger.info("Polling for linked tasks creation")
        poll_for_linked_tasks_creation(interval_minutes)
        logger.info("Polling for update linked tasks")
        poll_for_update_linked_tasks(interval_minutes)
        logger.info("Polling for update linked tasks forward")
        poll_for_update_linked_tasks_forward(interval_minutes)
        logger.info("Polling for update review tasks status")
        poll_for_update_review_tasks_status(interval_minutes)
        logger.info("Polling for update main tasks status")
        poll_for_update_main_tasks_status(interval_minutes)
        logger.info(f"Sleeping for {interval_minutes} minutes")
        time.sleep(interval_minutes * 60)

def schedule_log_cleanup():
    # Schedule the log cleanup function to run every day at 12 AM IST
    schedule.every().day.at("00:00").do(delete_old_log_records, log_dir='logs', days=2)
...
if __name__ == "__main__":
    ...
    try:
        poll_for_all_events(interval_minutes=1)  # Run polling continuously
